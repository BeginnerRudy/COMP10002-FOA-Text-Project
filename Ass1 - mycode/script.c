...
int stage1(int argc, char* argv[], word_t *);
int get_a_word(int c, word_t, char,int maybe_end,int len);
...
main(int argc, char *argv[]) {
    ...
    /*stage1*/
    k=stage1(argc, argv, query_words);
    ...
    while ((c = mygetchar()) != EOF ) {
        ...
        if (get_a_word(c, oneword,last_one_char,maybe_end,len)) {
            printf("%s",oneword);
            for (n=0;n<k;n++) {
...
stage1(int argc, char* argv[], word_t query_words[]) {
    int k=0,i,j,valid=1, invalid_query_index;
    if (argc<=1) {
        /*no input query, exit.*/
...
int
get_a_word(int c, word_t oneword, char last_one_char,int maybe_end,int len) {
